 ...
 *
 * Implements the FUSE callbacks for NUFS filesystem. This is the layer that
 * interacts with the FUSE library and translates FUSE calls into calls to the
 * storage layer.
 */
...
int nufs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  for (slist_t *cur = entries; cur != NULL; cur = cur->next) {
    filler(buf, cur->data,NULL,0);
  }
  slist_free(entries);
